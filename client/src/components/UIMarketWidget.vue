      ...
      <div class="stocks-scroll-area">
        ...
        >
          ...
          <div class="stocks-chart-container">
            ...
            >
              <path 
                :d="generateSparkline(item.change)" 
...
async function fetchProfiles() {
  ...
  for (const item of stockItems) {
    try {
      const response = await fetch(`https://finnhub.io/api/v1/stock/profile2?symbol=${item.symbol}&token=${props.apiKey}`);
...
async function fetchQuotes() {
  ...
  const promises = stocksData.value.map(async (item) => {
    try {
      const response = await fetch(`https://finnhub.io/api/v1/quote?symbol=${item.symbol}&token=${props.apiKey}`);
...
:global([data-theme="dark"]) .stocks-theme-switch::after {
...
/* Utility Colors */
.stocks-bg-blue { background: var(--stocks-color-blue); }
.stocks-bg-purple { background: var(--stocks-color-purple); }
.stocks-bg-orange { background: var(--stocks-color-orange); }
.stocks-bg-gray { background: var(--stocks-color-gray); }
.stocks-bg-black { background: #1c1c1e; }
...
.stocks-stroke-up { stroke: var(--stocks-color-green); }
.stocks-stroke-down { stroke: var(--stocks-color-red); }
