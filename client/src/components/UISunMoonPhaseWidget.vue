...
const sunDotFill = computed(() => {
  ...
  return now < start || now > end ? 'gray' : 'var(--sys-warning)'
})
...
const moonIllumination = computed(() =>
  moonData.value ? Math.round(moonData.value.fraction * 100) : 0,
)
...
onMounted(() => {
  timer = setInterval(() => (date.value = new Date()), 1000)
})
onUnmounted(() => clearInterval(timer))
</script>

<style lang="scss" scoped>
.sunmoon-header-actions {
  display: flex;
  align-items: center;
  gap: 8px;
}
