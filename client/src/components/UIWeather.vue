...
const toggleExpand = (forceState = null) => {
  ...
  if (newState) {
    ...
    // Calculation for animation start point
    ...
    nextTick(() => {
      ...
      // Remove specific top/left so CSS class handles centering
      widgetStyle.value = {
        position: 'fixed',
        zIndex: 50,
      }
...
watch(
  () => props.expanded,
  (newVal) => toggleExpand(newVal),
)

onMounted(() => {
  loadIcons()
  if (props.expanded) toggleExpand(true)
})
...
<template>
  <div class="weather-wrapper" :class="isDark ? 'weather-theme-dark' : 'weather-theme-light'">
    <!-- Backdrop -->
    <div class="backdrop" :class="{ active: isExpanded }" @click="toggleExpand(false)"></div>
      ...
      <div class="view-expanded">
        ...
        <div class="widget-header">
          ...
          <div class="header-actions">
            <button class="close-btn" @click.stop="toggleExpand(false)" title="Close">
              <i data-lucide="x" class="icon-sm"></i>
            </button>
            <div class="controls-row">
              <button class="icon-btn" @click.stop="isDark = !isDark">
                <i :data-lucide="isDark ? 'sun' : 'moon'" class="icon-xs"></i>
              </button>
...
.backdrop {
  position: fixed;
  inset: 0;
  background: var(--weather-backdrop);
...
.ios-widget.expanded .view-compact {
  opacity: 0;
  pointer-events: none;
  transition-delay: 0s;
}
.ios-widget.expanded .view-expanded {
  opacity: 1;
  pointer-events: all;
...
.expanded-content {
  ...
  flex-direction: column;
  flex: 1;
  min-height: 0; /* Important for scrolling flex child */
}
