...
const refreshData = async (silent = false) => {
  if (currentLocation.value.lat && currentLocation.value.lon) {
    await fetchWeather(currentLocation.value.lat, currentLocation.value.lon, currentLocation.value.name, silent)
  }
}

const startAutoRefresh = () => {
  if (refreshTimer.value) clearInterval(refreshTimer.value)
  refreshTimer.value = setInterval(() => { refreshData(true) }, 3600000)
}
...
const openWidget = async () => {
  ...
  modalStyle.value = {
    top: '50%',
    left: '50%',
    width: 'var(--weather-w-expand, 350px)',
    height: 'var(--weather-h-expand, 600px)',
...
onMounted(() => { loadIcons(); if (props.expanded) openWidget(); startAutoRefresh(); })
onUnmounted(() => { document.body.style.overflow = ''; if (refreshTimer.value) clearInterval(refreshTimer.value) })
</script>
          ...
          <div class="view-expanded">
            ...
            <div class="widget-header">
              ...
              <div class="header-actions">
                <button class="close-btn" @click.stop="closeWidget"><i data-lucide="x" class="icon-sm"></i></button>
...
.weather-wrapper { display: inline-block; width: 100%; height: 100%; }

/* Compact View overrides */
.compact-widget-view { cursor: pointer; transition: opacity 0.1s; height: 100%; }
.opacity-0 { opacity: 0; pointer-events: none; }
.compact-body { display: flex; flex-direction: column; justify-content: center; flex: 1; }
.compact-temp { font-size: 42px; font-weight: 300; line-height: 1; }
.compact-condition { font-size: 13px; color: var(--ui-text-secondary); font-weight: 500; margin-top: 4px; }
.icon-header { width: 20px; height: 20px; color: var(--text-color); }

/* Expanded Modal - kept mostly solid */
.weather-teleport-container { position: absolute; top: 0; left: 0; width: 0; height: 0; }
.overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.4); opacity: 0; transition: opacity 0.3s; pointer-events: none; }
.overlay.active { opacity: 1; pointer-events: auto; }
.expanded-modal {
  position: fixed; z-index: 9999; cursor: default;
  background: var(--weather-bg-expanded, #1c1c1e);
  border-radius: 24px; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.5);
  display: flex; flex-direction: column; overflow: hidden;
  transition: all 0.6s cubic-bezier(0.34, 1.56, 0.64, 1);
}
.view-expanded { padding: 24px; height: 100%; display: flex; flex-direction: column; overflow-y: auto; color: var(--text-color); }
...
.condition-lg { font-size: 15px; color: var(--ui-text-secondary); margin: 0; }
.header-actions { display: flex; flex-direction: column; align-items: flex-end; gap: 8px; }
.close-btn, .icon-btn { background: var(--weather-btn-bg, rgba(255,255,255,0.1)); border: none; width: 32px; height: 32px; border-radius: 50%; display: flex; align-items: center; justify-content: center; cursor: pointer; color: var(--text-color); }
...
.stats-row { display: flex; justify-content: space-around; background: rgba(255,255,255,0.05); border-radius: 16px; padding: 12px; margin-bottom: 20px; flex-shrink: 0; }
.stat-item { display: flex; flex-direction: column; align-items: center; gap: 4px; }
.stat-label { font-size: 10px; font-weight: 700; text-transform: uppercase; color: var(--ui-text-secondary); }
.stat-val { font-size: 14px; font-weight: 600; }
.forecast-container { border-top: 1px solid rgba(255,255,255,0.1); padding-top: 15px; flex-shrink: 0; padding-bottom: 10px; }
...
.f-day { font-size: 13px; font-weight: 600; color: var(--ui-text-secondary); }
.f-temp { font-size: 14px; font-weight: 600; }
.search-overlay { position: absolute; top: 70px; left: 20px; right: 20px; background: #333; padding: 10px; border-radius: 12px; display: flex; gap: 5px; transform: scale(0.95); opacity: 0; pointer-events: none; transition: all 0.2s; z-index: 60; }
...
.search-go { background: #007aff; color: white; border: none; padding: 0 12px; border-radius: 8px; cursor: pointer; }
.icon-lg { width: 80px; height: 80px; } .icon-sm { width: 20px; height: 20px; } .icon-xs { width: 14px; height: 14px; }
</style>
